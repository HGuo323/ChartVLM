...
class VQAFineTuneDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        input_ids = self.tokenizer.encode(
            f'chartqa: {sent}', max_length=400, truncation=True)
        ...
        if self.outputs is not None:
            ...
            if self.args.classifier:
                ...
                target_ids = self.tokenizer.encode(
                    answer, max_length=100, truncation=True)  # Changed 10 to 100
    ...
    def collate_fn(self, batch):
        ...
        if 'target' in batch[0]:
            # targets = []
            targets = torch.zeros(
                B, len(batch[0]['target']), dtype=torch.float)
...
class VQAEvaluator:
    ...
    def evaluate_raw(self, quesid2ans: dict, is_topk_optimal=None):
        ...
        for quesId, resAns in tqdm(quesid2ans.items(), total=len(quesid2ans), ncols=80):
            ...
            quesId = str(quesId)
            if quesId not in self.qidtoans:
                print(f"❗ Question ID not found in qidtoans: {quesId}")
                continue
